...
const RIGHE = 18
const COLONNE = 37
const URL = "http://localhost:3000/ombrelloni"
...
$(document).ready(function() {
    ...
    function isDisponibile(ombrellone) {
        let pos1 = (dataStart - new Date(_dataInizio.prop("min"))) / MMG
            //let pos2 = (dataEnd-new Date(_dataInizio.prop("min")))/MMG; formato iso/date : "yyyy-mm-dd"
    ...
    function ombrelloneClick() {
        ...
        if (!$(this).hasClass("blue")) {
            $(this).addClass("blue")
            vetPrenotazioni.push($(this).prop("id").split("-")[1])
        } else {
            $(this).removeClass("blue")
            let pos = vetPrenotazioni.indexOf(pos)($(this).prop("id").split("-")[1])
            vetPrenotazioni.splice(pos, 1)
        }
        console.log(vetPrenotazioni)
    }
    ...
    function creaPulsantePrenota() {
        ...
        a.on("click", function() {
            let pos1 = (dataStart - (new Date(_dataInizio.prop("min")))) / MMG
            let pos2 = (dataEnd - (new Date(_dataInizio.prop("min")))) / MMG // formato iso/date => yyyy-mm-dd    
            for (const id of vetPrenotazioni) {
                for (let i = pos1; i <= pos2; i++) {
            ...
            window.location.reload() // faccio il refresh della pagina
        })
    }
